...
3.  **DAG 拓扑调度器压测**：
    *   脚本在 `./tests_temp_dir/` 自动生成 200 个具有多叉 Fork-Join 依赖网的 `.http` 文件。
    *   调用 `rupost test --mode parallel --concurrency 50` 并发调度这 200 个文件，以 `time` 收集系统开销。
...
*   **DAG 并行批处理调度纯耗时**：运行 200 个带有 State Cloning 的本地回环（0ms 响应）用例，拓扑调度器的额外系统开销（剔除网络实际握手和发包时间）应限制在 **<= 20ms**。
