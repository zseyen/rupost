...
impl WsActionParser {
    ...
    pub fn parse_body(body_str: &str) -> Result<Vec<WsAction>, String> {
        ...
        while let Some(line) = lines.next() {
            ...
            if let Some(stripped) = trimmed.strip_prefix("SEND") {
                // 1. 发送帧动作
                let mut payload_str = stripped.trim().to_string();
            ...
            } else if let Some(stripped) = trimmed.strip_prefix("EXPECT") {
                // 2. 预期帧匹配动作
                let mut condition = stripped.trim().to_string();
                ...
                while let Some(next_line) = lines.peek() {
                    let next_trimmed = next_line.trim();
                    if let Some(stripped) = next_trimmed.strip_prefix("@timeout") {
                    ...
                    } else if let Some(stripped) = next_trimmed.strip_prefix("@assert") {
                        let _ = lines.next(); // 消费这一行
                        let content = stripped.trim();
                    ...
                    } else if let Some(stripped) = next_trimmed.strip_prefix("@capture") {
                        let _ = lines.next(); // 消费这一行
                        let content = stripped.trim();
                        let content = content.trim_start_matches('=').trim();
            ...
            } else if let Some(stripped) = trimmed.strip_prefix("WAIT") {
                // 3. 阻塞等待动作
