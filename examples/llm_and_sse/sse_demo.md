# RuPost 通用 Server-Sent Events (SSE) 接口测试与联调示例
...
使用 `@stream_to` 将接收到的 SSE 事件流内容实时追加或覆盖写入本地物理文件中。

```http
# @test
# @sse
# @stream_to ./target/build_log_demo.txt overwrite
POST http://127.0.0.1:8080/build-stream
Content-Type: application/json
