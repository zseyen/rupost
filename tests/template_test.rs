...
fn test_template_generation_http() {
    ...
    let http_path = temp_dir.path().join("test_llm.http");
    let env_path = temp_dir.path().join(".env.example");

    // 1. 生成 .http 模板
    let res = run_template("llm", Some(http_path.to_str().unwrap()), false, false);
    assert!(res.is_ok());
...
fn test_template_generation_markdown() {
    let temp_dir = TempDir::new().unwrap();
    let md_path = temp_dir.path().join("test_llm.md");
    let env_path = temp_dir.path().join(".env.example");

    // 1. 生成 .md 模板
    let res = run_template("llm", Some(md_path.to_str().unwrap()), false, false);
    assert!(res.is_ok());
...
fn test_template_safety_overwrite_protection() {
    ...
    // 4. 尝试带 --force 生成，预期成功
    let res_force = run_template("llm", Some(http_path.to_str().unwrap()), true, false);
    assert!(res_force.is_ok());

    // 5. 验证内容被成功覆盖，并且伴随生成了 .env.example
    let new_content = fs::read_to_string(&http_path).unwrap();
    assert!(new_content.contains("# RuPost 大模型 (LLM)"));
    assert!(env_path.exists());
}
...
fn test_template_list_command() {
    ...
    let res = run_template("llm", Some(path.to_str().unwrap()), false, true);
    assert!(res.is_ok());
    assert!(!path.exists());
}
...
fn test_default_output_path_resolution() {
    ...
    let res_llm = run_template("llm", None, false, false);
    assert!(res_llm.is_ok());
    assert!(temp_dir.path().join("llm_template.http").exists());
