...
async fn test_diagnostics_probe_localhost() {
    ...
    assert!(probe_res.is_ok(), "Probe should succeed on active mock server");
    ...
    assert!(tcp.as_millis() < 500, "TCP connect to localhost should be very fast");
}
...
async fn test_diagnostics_probe_public_url() {
    ...
    if let Ok((dns, tcp)) = probe_res {
        ...
        println!("Skipping public DNS/TCP verification due to offline/network environment constraints.");
    }
}
