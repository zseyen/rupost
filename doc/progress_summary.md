...
| **Sprint 2: 多文件与目录递归测试** | 递归扫描过滤、有向无环图 (DAG) 拓扑排序、顺序/并行双执行模式、并发 Cookie 隔离保护、Fail-Fast 流程中断、JSON 结构化报告 | 已完成 | `src/runner/scanner.rs`, `src/runner/workflow.rs`, `src/runner/batch.rs`, `tests/batch_testing_test.rs` |
...
3.  **BatchExecutor**：
    ...
    *   **Cookie 安全隔离**：并行模式下，为各 worker 分配独立的 Ephemeral CookieStore 及 Cloned Context 变量作用域副本，彻底消除多线程会话数据竞争。
