...
### 1.3 全局共享变量 (`global.`) 与系统 env 变量映射 (`env.`)
在批量并行测试中，各分支通常物理隔离（各自维护独立的 Cookie 和 Context），但在有拓扑依赖的链条中又需要进行鉴权令牌等信息的实时共享。
RuPost 设计了特殊的全局与环境变量前缀：
*   **全局共享变量 (前缀 `global.`)**：
    *   在用例中捕获全局共享变量：`@capture global.token from body.token`。
    *   在用例中引用全局共享变量：`{{global.token}}`。
    *   *技术实现*：在 Rust 内存中，全局共享变量保存在 `Arc<RwLock<HashMap<String, String>>>`。因为采用了带读写锁的原子引用计数锁，多线程的并行执行器能够跨线程安全地更新、获取同一个全局变量集，从而实现了跨文件的并发状态同步。
*   **系统环境直接映射 (前缀 `env.`)**：
    *   支持在用例中无需任何配置直接渲染本地系统的环境变量。例如：`{{env.PATH}}` 或 `{{env.USER}}`。
    *   *技术实现*：在变量求值时，一旦检测到 `env.` 前缀，系统会动态剥离前缀并直接从 `std::env::var` 级联获取最新的环境变量。

### 1.4 快速初始化 rupost.toml 模板 (`rupost init`)
无需从零手动创建 `rupost.toml` 配置文件，在项目根目录下直接运行：
```bash
rupost init
# 或者使用别名
rupost i
```
RuPost 将会自动在当前目录下生成一个格式标准的 `rupost.toml` 模板，其中预置了 `dev`、`prod` 环境以及带有详细注释的示例变量，方便您快速修改上手。
